...
import TaskListSNv from "../../Components/TaskListSNv";
...
export default function SeriesNv() {
  ...
  useEffect(() => {
    async function loadTask() {
      const taskStorange = await AsyncStorage.getItem("@serien");
  ...
  useEffect(() => {
    async function saveTask() {
      await AsyncStorage.setItem("@serien", JSON.stringify(task));
    }
    saveTask();
  }, [task]);
  ...
  return (
    <SafeAreaView style={styles.Container}>
      <View style={styles.titulo}>
        <Text style={styles.textTitulo}>Series Não Vistas</Text>
      </View>
      <AnimatedFl
        animation="fadeInUp"
        useNativeDriver
        ...
        renderItem={({ item }) => (
          <TaskListSNv data={item} handleDelete={handleDelete} />
        )}
      ></AnimatedFl>
